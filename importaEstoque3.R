carregaEstoque <- function(cod_cliente, pmascara_arquivo){
#setwd("C:/RAWDATA")
...
for (i in 1:length(estoque)){
      ...
      if (!is.na(as.numeric(substr(estoque[i],1,6)))&&!is.na(as.numeric(substr(estoque[i],98,106)))){
            ...
            filial <- as.numeric(substr(estoque[i],15,16))
