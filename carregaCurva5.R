carregaCurva <- function(data='20161018',data_inicial='20161012',data_inicial_filial3="20161012",puser="cedot"){
####################################################################
##
...
# fator de ajuste futuro: crescimento de 20%% / em relação ao período médio (vida da loja / 2)
fatorAjusteFuturo    <- 1.2^((numero_de_meses/2)+0.5)
fatorAjusteFuturoF03 <- 1.2^((numero_de_mesesF03/2)+0.5)
...
curvaItens$cobertura_semanas[curvaItens$filial %in% c(14,15)] <- curvaItens$est[curvaItens$filial %in% c(14,15)]/((curvaItens$qtd_venda[curvaItens$filial %in% c(14,15)]/numero_de_semanas)*fatorAjusteFuturo)
curvaItens$cobertura_semanas[curvaItens$filial==16]          <- curvaItens$est[curvaItens$filial==16]/((curvaItens$qtd_venda[curvaItens$filial==16]/numero_de_semanasF03)*fatorAjusteFuturoF03)
...
demanda$dem3sem[demanda$filial %in% c(14,15)] <- ((demanda$qtd_venda[demanda$filial %in% c(14,15)]/numero_de_semanas)*fatorAjusteFuturo*3)-(demanda$est[demanda$filial %in% c(14,15)])
demanda$dem3sem[demanda$filial %in% c(16)]   <- ((demanda$qtd_venda[demanda$filial %in% c(16)]/numero_de_semanas)*fatorAjusteFuturo*3)-(demanda$est[demanda$filial %in% c(16)])

demanda$dem6sem[demanda$filial %in% c(14,15)] <- ((demanda$qtd_venda[demanda$filial %in% c(14,15)]/numero_de_semanas)*fatorAjusteFuturo*6)-(demanda$est[demanda$filial %in% c(14,15)])
demanda$dem6sem[demanda$filial %in% c(16)]   <- ((demanda$qtd_venda[demanda$filial %in% c(16)]/numero_de_semanas)*fatorAjusteFuturo*6)-(demanda$est[demanda$filial %in% c(16)])
