...
#USER                <- "cdottori"
USER                <- "lgiaretta"
DATA_INICIAL        <- "20161012"  #início das operações para cálculo específico
COD_CLIENTE         <- 2
...
source("processaCompras2.R")
dadosCompra <- XL7_Processa_compra(cliente=COD_CLIENTE,puser=USER,pdir=working_dir)
histCompra  <- read.csv2("datasetCompra.csv",sep=";",stringsAsFactors = FALSE)

setwd(working_dir)

# chama scripts para tratamento dos arquivos
source("insereProdutosSKUs2.R")
source("importaTipoProduto2.R")
source("carregaCurva5.R")
demanda2 <- carregaCurva(data = DATA,puser=USER, data_inicial=DATA_INICIAL)

source("importaEstoque3.R")
carregaEstoque(cod_cliente=COD_CLIENTE, pmascara_arquivo=DATA)

source("minimoDemanda2.R")
...
curvaSplit1 <- curvaEstoque[curvaEstoque$COD_FILIAL_CLIENTE==14,c("COD_INTERNO","qtd","qtd_venda","cobertura_semanas")]
curvaSplit2 <- curvaEstoque[curvaEstoque$COD_FILIAL_CLIENTE==15,c("COD_INTERNO","qtd","qtd_venda","cobertura_semanas")]
curvaSplit3 <- curvaEstoque[curvaEstoque$COD_FILIAL_CLIENTE==16,c("COD_INTERNO","qtd","qtd_venda","cobertura_semanas")]
names(curvaSplit1)[c(2,3,4)]<-c("est_fl14","venda_fl14","cobert_sem_fl14")
names(curvaSplit2)[c(2,3,4)]<-c("est_fl15","venda_fl15","cobert_sem_fl15")
names(curvaSplit3)[c(2,3,4)]<-c("est_fl16","venda_fl16","cobert_sem_fl16")
