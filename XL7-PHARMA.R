...
XL7_ImportaMovimentacao_SoftPharma <- function(cliente,path){
      ...
      for (i in 1:nrow(movimentacao)){
            ...
            } else if (length(grep("\\| Período: ",movimentacao[i,]))!=0) {
                  
                  dia<-substr(strsplit(movimentacao[i,],"Período: ")[[1]][2],1,10)
            }      
...
XL7_Gera_Report_Vendas<-function(cliente,dia){
    ...
    movimentacao<-XL7_ImportaMovimentacao_SoftPharma("2",working_dir)
    write.csv2(movimentacao,paste0(working_dir,"/movimentacaoDescontoPopular.csv"),row.names = F)
    
    acompanhamento<-XL7_ImportaAcompanhamentoDiario_SoftPharma("2",working_dir)
    write.csv2(acompanhamento,paste0(working_dir,"/acompanhamentoDescontoPopular.csv"),row.names = F)
    ...
    for (f in c("TODAS",filial)){
        ...
        rmarkdown::render(paste0(gsub("/Desconto Popular","/R",working_dir),"/DescontoPopular_Report.Rmd"), 
